...
class BitFlipChannel(Channel):
    ...
    def get_param(self,ind):
        return [(1 - self.px[ind])*(1-self.pz[ind]),(1-self.px[ind])*self.pz[ind],(1-self.pz[ind])*self.px[ind],self.px[ind]*self.pz[ind]]
...
class PauliChannel(Channel):
    ...
    def channel(self,n=-1,ind=0):
        ...
        if n>0:
            self.n = n
        x_pos = np.where(np.random.random(self.n) <= self._channel_parameter["px"][ind])[0]
        z_pos = np.where(np.random.random(self.n) <= self._channel_parameter["pz"][ind])[0]
        self._channel_output["E"][x_pos] = 1  # X
        self._channel_output["E"][self.n + z_pos] = 1  # Z
        return self._channel_output
    ...
    def get_param(self,ind):
        ...
        return [(1 - px)*(1-pz),(1-pz)*px,(1-px[ind])*pz,px[ind]*pz[ind]]
